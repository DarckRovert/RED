...
export class AcousticSonarEngine {
    ...
    public async emitPing(medium: SonarMediumType = this.currentMedium): Promise<SonarPingResult> {
        ...
        if (this.isPinging) {
            return this.lastResult || { distanceMeters: 0, timeOfFlightMs: 0, medium, confidencePct: 0, timestamp: Date.now() };
        }
        this.isPinging = true;
        ...
        try {
            ...
            if (micReady && this.echoAnalyser) {
                ...
                for (let w = 0; w < totalListenWindows; w++) {
                    if (!this.echoAnalyser || !this.audioCtx) break;
            ...
            if (this.echoAnalyser && this.audioCtx) {

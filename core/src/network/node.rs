...
use crate::identity::{AuthorizedDevice, DeviceId, DevicePublicKey, Identity, IdentityHash};
use crate::network::routing::OnionRouter;
use crate::network::transport::Transport;
use crate::network::{Libp2pTransport, NetworkConfig, NetworkError, NetworkResult, PeerId};
use crate::protocol::{
    Conversation, ConversationId, Group, GroupId, GroupMember, MemberRole, Message, MessageId,
    MessageType,
};
...
impl Node {
    ...
    pub async fn set_dms_config(
        ...
        let _ = storage.set_config(
            "dms_wipe_messages",
            if wipe_messages { "true" } else { "false" },
        );
        let _ = storage.set_config(
            "dms_wipe_identity",
            if wipe_identity { "true" } else { "false" },
        );
    ...
    pub async fn start(node_ref: Arc<Mutex<Self>>) -> NetworkResult<()> {
        ...
        info!(
            "Starting RED node with identity: {}",
            n.identity.identity_hash().short()
        );
        ...
        let default_port = if cfg!(windows) {
            "COM3".into()
        } else {
            "/dev/ttyUSB0".into()
        };
        let mut lora =
            crate::network::lora_bridge::LoraBridge::new(node_ref.clone(), default_port, 115200);
    ...
    pub async fn attach_lora_bridge(
        ...
        info!(
            "📻 [LoRa PnP] Enlazando transceptor físico en {} @ {} bps...",
            port, baud_rate
        );
        let mut lora =
            crate::network::lora_bridge::LoraBridge::new(node_ref.clone(), port, baud_rate);
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                match s.prune_expired_messages() {
                    Ok(count) if count > 0 => {
                        info!("Background prune: removed {} expired messages", count)
                    }
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                let _ = transport
                    .send(
                        &dummy_peer,
                        crate::network::transport::TransportMessage::IdentityBroadcast {
                            hash: hash_hex,
                            pk: pk_hex,
                        },
                    )
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                for (key, message) in pending {
                    ...
                    let is_recipient_online = available_peers
                        .iter()
                        .any(|p| p.identity_hash.as_ref() == Some(&message.recipient));

                    if is_recipient_online {
                        info!(
                            "Peer {} came online. Retrying delivery of message {}",
                        ...
                        match deliver_res {
                            Ok(_) => {
                                info!(
                                    "Delivery of pending message {} succeeded.",
                                    message.id.to_hex()
                                );
            ...
            loop {
                ...
                for peer_id in peer_ids {
                    let _ = transport_clone
                        .send(
                            &peer_id,
                            crate::network::transport::TransportMessage::Data {
                    ...
                    tokio::spawn(async move {
                        ...
                        match msg {
                            ...
                            crate::network::transport::TransportMessage::Data { payload } => {
                                ...
                                if let Ok(packet) = bincode::deserialize::<
                                    crate::network::routing::OnionPacket,
                                >(&payload)
                                {
                                    n.handle_onion_packet(peer_id, packet).await;
    ...
    async fn handle_onion_packet(
        &mut self,
        _from: PeerId,
        mut packet: crate::network::routing::OnionPacket,
    ) {
        ...
        let secret = self
            .identity
            .key_exchange(&crate::crypto::keys::PublicKey::from_bytes(
                layer.ephemeral_pk,
            ));
        ...
        match self.onion_router.peel_layer(layer, &secret).await {
            Ok(routing_info) => {
                if let Some(next_hop_addr) = routing_info.next_hop {
                    ...
                    debug!(
                        "Mixnet Active: Obfuscating metadata, delaying transmission by {}ms",
                        delay_ms
                    );
    ...
    async fn handle_gossip_message(&mut self, _sender: PeerId, data: Vec<u8>) {
            ...
            if let Ok(json_str) = std::str::from_utf8(payload) {
                if let Ok(parsed) = serde_json::from_str::<serde_json::Value>(json_str) {
                    if let Some(content_str) = parsed.get("content").and_then(|c| c.as_str()) {
                        let sender_hex =
                            parsed.get("sender").and_then(|s| s.as_str()).unwrap_or("");
                        let recipient_hex = parsed
                            .get("recipient")
                            .and_then(|r| r.as_str())
                            .unwrap_or("");
        ...
        if let Ok(packet) = bincode::deserialize::<crate::network::routing::OnionPacket>(&data) {
            ...
            error!(
                "Failed to deserialize gossip message as MeshPacket, JSON, OnionPacket or Message"
            );
        }
    }
    ...
    pub async fn handle_incoming_message(&mut self, mut message: Message) {
        ...
        // Drop messages from blocked senders (Security blocklist check)
        {
            ...
            if let Some(contact) = s.get_contact(&message.sender) {
                if contact.blocked {
                    info!(
                        "Discarding message from blocked sender: {}",
        ...
        if message.recipient != my_hash && message.sender != my_hash && !is_public {
            trace!(
                "Discarding gossipsub message not addressed to us: {} -> {}",
                message.sender.short(),
                message.recipient.short()
            );
            return;
        ...
        if let MessageType::ChannelHopCoordination {
            target_channel,
            frequency_mhz,
            ref reason,
            ..
        ...
        {
            self.rf_current_channel
                .store(target_channel, std::sync::atomic::Ordering::SeqCst);
            self.rf_hops_count
                .fetch_add(1, std::sync::atomic::Ordering::SeqCst);
            info!(
                "Synchronized mesh channel hop to #{} ({} MHz) - Reason: {}",
                target_channel, frequency_mhz, reason
            );
        }
        ...
        if let MessageType::GroupPayload(ref group_msg) = message.content {
            ...
            if let Some(mut group) = s.get_group(&group_msg.group_id) {
                ...
                error!(
                    "Received group message for unknown group {:?}",
                    group_msg.group_id
                );
            }
        }
        ...
        if let MessageType::GroupInvite {
            ref group_id,
            ref group_name,
            ref creator_hash,
            ref members,
        ...
        } = message.content
        {
            ...
            if s.get_group(&gid).is_none() {
                info!(
                    "Auto-registering incoming GroupInvite for '{}' ({:?})",
                    group_name, gid
                );
                    ...
                    if m_hash != creator_hash {
                        ...
                        let _ = new_group.add_member(crate::protocol::GroupMember {
                            ...
                            public_key: if is_me {
                                ...
                                crate::crypto::keys::PublicKey::from_bytes([0u8; 32])
                            },
                            joined_at: created_at,
        ...
        // Save to storage
        {
            ...
            if let MessageType::ProfileSyncResponse { avatar, bio } = &message.content {
                if let Some(mut contact) = s.get_contact(&message.sender) {
                    ...
                    contact.last_sync = std::time::SystemTime::now()
                        .duration_since(std::time::UNIX_EPOCH)
                        .unwrap_or_default()
                        .as_secs();
            ...
            if let MessageType::SocialPost(data) = &message.content {
                if let Ok(payload) =
                    serde_json::from_slice::<crate::protocol::SocialPostPayload>(data)
                {
            ...
            if let MessageType::P2PVoucher(data) = &message.content {
                if let Ok(payload) =
                    serde_json::from_slice::<crate::protocol::P2PVoucherPayload>(data)
                {
                    if s.get_p2p_voucher(&payload.id).is_none() {
                        ...
                        if let Ok(mut wallet) = s.get_p2p_wallet() {
                            ...
                            info!(
                                "Credited incoming P2P voucher {} ({} RED credits)",
            ...
            if let MessageType::MedicalTriageReport(data) = &message.content {
                if let Ok(payload) =
                    serde_json::from_slice::<crate::protocol::MedicalTriagePayload>(data)
                {
                    ...
                    info!(
                        "🚨 Stored incoming Medical Triage Report: {} ({}) evaluated by {}",
                        payload.victim_label, payload.category, payload.evaluator_name
                    );
                }
            }

            // Handle EmergencyBeacon (v36.0 Tactical SOS Mesh Broadcast)
            if let MessageType::EmergencyBeacon {
                ref beacon_id,
                ref distress_type,
                latitude,
                longitude,
            ...
            if let MessageType::TimerUpdate { seconds } = message.content {
                ...
                if let Some(mut conv) = s.get_conversation_mut(&conv_id) {
                    ...
                    info!(
                        "Updated disappearing timer to {}s for conversation {}",
                        seconds, conv_id
                    );
                }
            }
    ...
    pub async fn send_group_message(
        ...
        let my_hash = self.identity.identity_hash().clone();
        {
            ...
            if let Err(e) = s.add_message(outbound_msg) {
                error!(
                    "Failed to save outgoing group message to local storage: {:?}",
                    e
                );
            }
        }
        ...
        for member in members {
            ...
            if let Err(e) = self
                .send_message(member.identity_hash.clone(), outer_msg)
                .await
            {
                error!(
                    "Failed to route group message to member {}: {:?}",
                    member.identity_hash.short(),
                    e
                );
            }
        }
    ...
    pub async fn send_message(
        ...
        let is_control_or_status = message.content.is_control()
            || matches!(
                message.content,
                MessageType::StatusPacket(_)
                    | MessageType::SocialPost(_)
        ...
        match self.deliver_message(&recipient, &message).await {
            ...
            Err(e) => {
                tracing::warn!(
                    "Message delivery to {} failed ({:?}). Queuing in pending deliveries.",
                    recipient.short(),
                    e
                );
                let s = self.storage.lock().await;
    ...
    async fn deliver_message(
        ...
        let local_destination = available_peers
            .iter()
        ...
        if let Some(destination) = local_destination {
            ...
            info!(
                "[deliver] Recipient {} found in local mesh → direct 1-hop path",
                &recipient.to_hex()[..8]
            );
            ...
            let packet = self
                .onion_router
                .create_packet(&single_hop_route, &payload, &[shared_secret], my_pub_bytes)
                .map_err(|e| {
                    NetworkError::TransportError(format!("1-hop encrypt failed: {}", e))
                })?;
            ...
            let _ = self
                ...
                .send(
                    ...
                    TransportMessage::Data {
                        payload: bincode::serialize(&packet).unwrap_or_default(),
                    },
                )
                .await;
            info!(
                "[deliver] 1-hop encrypted Onion packet dispatched to {}",
                &recipient.to_hex()[..8]
            );
        } else {
            ...
            if relay_peer_count >= 3 {
                ...
                .await
                .unwrap_or(Err(NetworkError::RoutingFailed(
                    "Timeout resolving peer".to_string(),
                )))?;

                let updated_peers = self.transport.known_peers();
                let destination = updated_peers
                    .into_iter()
                ...
                let packet = self
                    .onion_router
                    .create_packet(&route, &payload, &shared_secrets, my_pub_bytes)
                    .map_err(|e| NetworkError::TransportError(e.to_string()))?;

                let first_hop = &route.hops[0].peer_id;
                use crate::network::transport::TransportMessage;
                self.transport
                    .send(first_hop, TransportMessage::Onion(packet.clone()))
                    .await?;
                info!(
                    "[deliver] Onion packet sent to first hop {} (multi-hop circuit)",
                    first_hop.to_hex()
                );
                ...
                info!(
                    "[deliver] Recipient not in local mesh, <3 relay peers → WAN best-effort path"
                );
                ...
                if let Ok(packet) = self.onion_router.create_packet(
                    &single_hop_route,
                    &payload,
                    &[shared_secret],
                    my_pub_bytes,
    ...
    pub fn rotate_identity(&mut self) -> NetworkResult<()> {
        if let Ok(new_identity) = self.identity.rotate() {
            tracing::info!(
                "Rotating node identity from {} to {}",
                self.identity.identity_hash().to_hex(),
                new_identity.identity_hash().to_hex()
            );
            self.identity = new_identity;
            Ok(())
        } else {
            Err(NetworkError::IoError(std::io::Error::other(
                "Failed to rotate identity",
            )))
        }
    }
    ...
    pub async fn mark_conversation_read_in_storage(
        &self,
        id: &ConversationId,
    ...
    pub async fn send_group_invite(
        ...
        let (group_name, members, created_at) = {
            ...
            if let Some(group) = s.get_group(&group_id) {
                let member_hashes: Vec<IdentityHash> =
                    group.members().map(|m| m.identity_hash.clone()).collect();
    ...
    pub async fn authorize_device(
        ...
        name: String,
        code: String,
    ) -> NetworkResult<AuthorizedDevice> {
    ...
    pub async fn get_sync_payload(
        &self,
    ) -> NetworkResult<(Vec<crate::storage::Contact>, Vec<Group>, Vec<Conversation>)> {
        let s: tokio::sync::MutexGuard<'_, Storage> = self.storage.lock().await;
    ...
    pub async fn unblock_contact(&self, hash: &crate::identity::IdentityHash) -> NetworkResult<()> {
        let mut s = self.storage.lock().await;
        s.unblock_contact(hash)
            .map_err(|e| NetworkError::TransportError(e.to_string()))
    }

    /// Toggle verification status of a contact
    pub async fn toggle_verify_contact(
        &self,
        hash: &crate::identity::IdentityHash,
    ) -> NetworkResult<bool> {
        let mut s = self.storage.lock().await;
        s.toggle_verify_contact(hash)
            .map_err(|e| NetworkError::TransportError(e.to_string()))
    }
}

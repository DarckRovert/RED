...
pub fn peel_onion(
    ...
    // Decrypt payload
    let decrypted = decrypt(&key, &packet.payload)
...
fn encrypt_layer(
    ...
    // Compute shared secret
    let shared_secret = x25519_diffie_hellman(&ephemeral.secret, recipient_pk);
    ...
    let packet = OnionPacket::new(ephemeral.public, ciphertext, mac);

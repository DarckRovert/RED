# 📋 Lista de Verificación de Release (Release Checklist) — RED v104.0.0
...
- [x] Versión **`v104.0.0`** unificada en 22 archivos maestros (`version.ts`, `build.gradle`, Cargo workspaces, service workers y documentación).
- [x] `ARCHITECTURE.md` → Sincronizado a v104.0.0 con SSOT de protocolo documentado.
- [x] `GOVERNANCE.md` → Sincronizado a v104.0.0 con Regla 1.5 (Nivel 2 SSOT & Cero Duplicación Backend).
- [x] `CHANGELOG.md` → Entrada de v104.0.0 registrando Tactical Command HUD & Complete Autonomous Triage Integration.
- [x] `SHA256SUMS.txt` → Hash `ECD9320D9433AE86F2C8FBCB1D658EDB09928A02C7CFD31FEFA53B78093EAF73` para `red-v104.0.0-release.apk` y `red-latest.apk`.
...
- [x] **APK Android v104.0.0:** `release-assets/red-v104.0.0-release.apk` (63.43 MB) — SHA-256: `ECD9320D9433AE86F2C8FBCB1D658EDB09928A02C7CFD31FEFA53B78093EAF73`.
- [x] **Verificación de Sumas de Comprobación:** Checksums SHA-256 publicados en `SHA256SUMS.txt` y `release-assets/RED-v104.0.0.apk.sha256`.
- [x] **Validación en Hardware Real:** Desinstalación higiénica e instalación limpia en Motorola Moto G22 (`ZT322B386P`). Ejecución en primer plano con 0 crashes en Logcat, telemetría de sensores en vivo (GPS, PDR, brújula magnética, radar BLE) y nodo Rust libp2p inicializado.

### Etiquetado y Despliegue Git
- [x] Crear y empujar el tag anotado y firmado:
  ```bash
  git tag -a v104.0.0 -m "RED Sovereign Mesh OS v104.0.0 (Tactical Command HUD & Complete Autonomous Triage Integration)"
  git push origin v104.0.0
  ```
- [x] Validar que GitHub Pages ([darckrovert.github.io/RED/](https://darckrovert.github.io/RED/)) responda HTTP 200.
- [x] Publicar assets y notas de release en GitHub Releases v104.0.0 ([Releases v104.0.0](https://github.com/DarckRovert/RED/releases/tag/v104.0.0)).

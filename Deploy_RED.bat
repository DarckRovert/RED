...
:: Proposito: Sincronizacion del motor RED (v7.1.0 Mesh)
:: ==========================================================
...
set COMMIT_MSG=feat: RED Mesh Protocol synchronization %FECHA% %HORA%

echo Procediendo con la sincronizacion del protocolo RED...
echo.

:: 3. Ejecutar Despliegue
echo [RED] Verificando cambios...
echo [RED] --- Start Check: %TIME% --- >> "%LOG_FILE%"
...
git remote set-url origin "https://github.com/DarckRovert/RED.git" >> "%LOG_FILE%" 2>&1
...
if %CHANGES_FOUND% EQU 0 (
    echo [SKIP] No se detectaron cambios en el protocolo. RED en estado soberano.
    echo [SKIP] No changes found >> "%LOG_FILE%"
    timeout /t 3 >nul
    exit /b 0
)

echo [RED] Cambios detectados. Iniciando sincronizacion...
echo %COMMIT_MSG% > LAST_DEPLOY_RED.txt
...
if %ERRORLEVEL% EQU 0 (
    ...
    echo ==========================================================
    echo [OK] Protocolo RED sincronizado exitosamente.
    echo [OK] Red Encriptada Descentralizada actualizada.
    echo ==========================================================
) else (
    echo.
    echo ==========================================================
    echo [!] FALLO CRITICO EN EL DESPLIEGUE DE RED
    echo ==========================================================
    echo [TIP]: Revisa deploy_log.txt para mas detalles técnicos.
)

echo [RED] --- End: %TIME% --- >> "%LOG_FILE%"
echo.
pause
exit /b 0

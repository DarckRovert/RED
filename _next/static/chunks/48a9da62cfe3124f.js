...
${r}`}static createBlobUrl(e){let t=new Blob([this.compileBundleToHtml(e)],{type:"text/html;charset=utf-8"});return URL.createObjectURL(t)}static exportBundle(e,t){return JSON.stringify({manifest:{...e,updatedAt:Date.now()},files:t})}static importBundle(e){try{let t=JSON.parse(e);if(!t.manifest||!t.manifest.id||!t.files)throw Error("El archivo .redapp no tiene un manifiesto o archivos válidos.");return t}catch(e){throw Error(`Error al procesar paquete .redapp: ${e.message}`)}}}e.s(["RedAppBundleEngine",()=>t])}]);
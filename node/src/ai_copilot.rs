    ...
    fn execute_inference(
        ...
        let path_buf = if !clean_path.is_empty() && Path::new(clean_path).exists() {
            ...
            if let Some(d) = dirs::data_local_dir() {
                candidate_dirs.push(d.join("RED").join("models"));
            }
            if let Some(d) = dirs::home_dir() {
                candidate_dirs.push(d.join(".red").join("models"));
            }
            candidate_dirs.push(PathBuf::from("models"));
            candidate_dirs.push(PathBuf::from("files/models"));
            candidate_dirs.push(PathBuf::from("client/app/models"));
            candidate_dirs.push(PathBuf::from("client/app/public/models"));
        ...
        if let Ok(meta) = std::fs::metadata(path) {
            const MAX_SAFE_DESKTOP_MODEL_BYTES: u64 = 8_500_000_000; // ~8.5 GB límite seguro en nodo de escritorio
            if meta.len() > MAX_SAFE_DESKTOP_MODEL_BYTES {
                return CopilotResponse {
                    answer: format!(
        ...
        impl LocalModel {
            fn forward(&mut self, input: &Tensor, start_pos: usize) -> candle_core::Result<Tensor> {
                match self {
        ...
        // Formateo de ChatML / Instruct formal según la arquitectura e inyección de contexto RAG
        ...
        let mut logits_processor =
            candle_transformers::generation::LogitsProcessor::new(299792458, Some(0.35), Some(0.9));
        ...
        for index in 0..max_tokens {
            ...
            let input = match Tensor::new(&tokens[start_pos..], &device)
        ...
        CopilotResponse {
            ...
            source: "RED Sovereign Desktop Node (Candle)".to_string(),
            model_used: model_name,
            execution_time_ms: start.elapsed().as_millis() as u64,
        }
    }

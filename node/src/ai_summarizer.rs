    ...
    pub fn summarize(&self, req: SummarizeChannelRequest) -> ChannelSummaryResponse {
        ...
        if total == 0 {
            return ChannelSummaryResponse {
                ...
                summary_bullets: vec![
                    "No hay mensajes recientes en el canal para sintetizar.".to_string()
                ],
        ...
        let stopwords: HashSet<&str> = vec![
            ...
            "para", "con", "no", "una", "su", "al", "lo", "como", "más", "pero", "sus", "le", "ya",
            ...
            "nuestra", "nuestros", "nuestras", "vuestro", "vuestra", "vuestros", "vuestras", "es",
            "son",
        ]
        .into_iter()
        .collect();
        ...
        for (idx, msg) in req.messages.iter().enumerate() {
            ...
            for word in &words {
                ...
                if alert_words.contains(&word.as_str()) {
                    alert_count += 1;
                }
        ...
        let mut scored_msgs: Vec<(usize, usize)> = msg_scores
            .into_iter()
            .map(|(idx, unique_words)| {
                let score = unique_words
                    .iter()
                    .map(|w| word_freq.get(w).unwrap_or(&1))
                    .sum::<usize>();
                (idx, score)
            })
            .collect();
